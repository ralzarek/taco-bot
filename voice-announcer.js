...
VoiceAnnouncer.prototype.on = function(serverId, callback) {
	if(this.servers.indexOf(serverId) < 0) {
...
VoiceAnnouncer.prototype.off = function(serverId, callback) {
	var index = this.servers.indexOf(serverId);
	if (index !== -1) {
...
VoiceAnnouncer.prototype.status = function(serverId, callback) {
	var index = this.servers.indexOf(serverId);
	if (index !== -1) {
		callback('Voice announce is on!');
	} else {
...
VoiceAnnouncer.prototype.joined = function(channel, user) {
	...
	fs.exists(path, function(exists) {
		if(!exists) {
			self.createTTSFile(name + ' has joined.', path, function() {
				self.enqueue({'userId': user.id, 'channel': channel, 'path': path});
			});
		} else {
			self.enqueue({'userId': user.id, 'channel': channel, 'path': path});
		}
